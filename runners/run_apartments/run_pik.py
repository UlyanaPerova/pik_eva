#!/usr/bin/env python3
"""
Запуск парсера квартир ПИК + экспорт в xlsx.

Использование:
    python runners/apartments/run_pik.py
"""
...
from parsers.pik_apartments import PikApartmentParser
...
async def main() -> int:
    logger.info("=" * 50)
    logger.info("Запуск парсера квартир ПИК")
    logger.info("=" * 50)
    ...
    try:
        ...
        for r, data in stats["by_rooms"].items():
            logger.info("    %s: %d шт., ср. цена: %s ₽, ср. цена/м²: %s ₽",
