...
def validate_config(
    ...
    *,
    ...
    for idx, entry in enumerate(links):
        ...
        # --- Идентификатор записи: object_id, url или project ---
        obj_id = entry.get("object_id")
        has_url = bool(entry.get("url"))
        has_project = bool(entry.get("project"))
        has_identifier = obj_id is not None or has_url or has_project
        if not has_identifier:
            errors.append(f"{prefix}: отсутствует 'object_id', 'url' или 'project'")
        ...
        # --- complex_name (обязательное для ДОМ.РФ, необязательное для URL/project — парсится со страницы) ---
        cname = entry.get("complex_name")
        if cname is None and not has_identifier:
            errors.append(f"{prefix}: отсутствует обязательное поле 'complex_name'")
        elif cname is not None and (not isinstance(cname, str) or not cname.strip()):
        ...
        if require_building and building is None and not has_url and not has_project:
        ...
        if not has_url and not has_project:
